...
<header class="px-6 py-4 shadow flex items-center gap-3 text-white" style="background:#0053e2">
  <div>
    <h1 class="text-lg font-bold">AI POD Verification Dashboard <span class="font-normal text-blue-200">— created by Harshal Bhuse — LMD Delivery Optimization</span></h1>
...
<main class="max-w-7xl mx-auto px-6 py-6 space-y-6">
  ...
  <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
    ...
    <div class="bg-white rounded-xl shadow p-5 text-center">
      <div class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Missing Orders</div>
      <div class="text-xs text-gray-400 mb-1">% of all PODs</div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-xl shadow p-5">
      ...
      <p class="text-xs text-gray-400 mb-4">Of drivers with each violation, what % of all their PODs were missing orders?</p>
      <div class="chart-wrap"><canvas id="c_mp_by_type"></canvas></div>
    </div>
  </div>
...
function renderKPIs(rows) {
  ...
  document.getElementById('kpi_missing').textContent      = pods ? (mp/pods*100).toFixed(1)+'%' : '—';
}
...
function renderMpByType(rows) {
  ...
  const labels = ['Inside Vehicle','Suspected Fraud','Profanity','Acceptable PODs'];
  const data   = [
    mpPct(r => r.iv > 0),
    mpPct(r => r.fr > 0),
    mpPct(r => r.pr > 0),
  ...
  const colors = ['#0053e2','#ea1100','#ffc220','#2a8703'];
  const cfg = {
    type: 'bar',
